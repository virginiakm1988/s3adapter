...
class EnsembleRunner():
    ...
    def compare_sequence(self, s1, s2):
        ...
        dp[0][0] = np.linalg.norm(t1[0] - t2[0], ord=2)
        for i in range(t1):
            for j in range(t2):
                if not j and not i:
                    continue
                if not j:
                    dp[i][j] = dp[i - 1][j] + np.linalg.norm(t1[i] - t2[j], ord=2)
                    frm[i][j] = (i - 1, j)
                elif not i:
                    dp[i][j] = dp[i][j - 1] + np.linalg.norm(t1[i] - t2[j], ord=2)
                    frm[i][j] = (i, j - 1)
                else:
                    for di, dj in [(-1, 0), (-1, -1), (0, -1)]:
                        nx, ny = i + di, j + dj
                        if dp[nx][ny] + np.linalg.norm(t1[i] - t2[j], ord=2) < dp[i][j]:
                            frm[i][j] = (nx, ny)
                            dp[i][j] = dp[nx][ny] + np.linalg.norm(t1[i] - t2[j], ord=2)
