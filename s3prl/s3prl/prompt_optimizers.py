...
def get_prompt_optimizer(model_params, prompt_lst, total_steps, optimizer_config):
...
def get_TorchOptim(model_params, prompt_lst, torch_optim_name, **kwargs):
    params = []
    for m in model_params:
        params += list(m.parameters())
   
    print("Add prompt list to Optimizer!")
    # params += prompt_lst ###
    Opt_class = getattr(torch.optim, torch_optim_name)
    kwargs.pop('total_steps')
    if 'prompt_lr' in kwargs:
        prompt_lr = kwargs['prompt_lr']
        kwargs.pop('prompt_lr')
    else:
        prompt_lr = kwargs['lr']
    
    if "prefix" in sys.argv[-1]:
        print(f'Prompt lr: {prompt_lr}!')
        optim = Opt_class([{'params': params}, {'params': prompt_lst, 'lr': prompt_lr}], **kwargs)
    else:
