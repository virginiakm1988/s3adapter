...
class Runner():
    ...
    def _get_upstream(self):
        ...
        #print("upstream model path", ckpt_path)
        model = Upstream(
            ckpt = ckpt_path,
    ...
    def train(self):
        ...
        for entry in self.all_entries:
            ...
            print("SELF NAME",self.args.expname, "bitfit" in self.args.expname, "bitfit" in self.args.expname and entry.name == "Upstream")
            
                    
            if self.args.adapter == True and entry.name == "Upstream":
            ...
            if "bitfit" in self.args.expname and entry.name == "Upstream":
                    for  name, param in entry.model.named_parameters():
                            if "bias" in name:
                                param.requires_grad = True
                                print("BITFIT",name)
        ...
        while pbar.n < pbar.total:
            ...
            path = os.path.join(self.args.expdir,'output.txt')
            f = open(path, 'w')
            print("epoch", epoch,time.time()-start_time," second", file=f)
    ...
    def push_to_huggingface_hub(self):
        ...
        if len(checkpoints) == 0:
            ...
            print(f"[Runner] - Found  checkpoint {checkpoints[0]}!")
            CKPT_PATH = checkpoints[0]
