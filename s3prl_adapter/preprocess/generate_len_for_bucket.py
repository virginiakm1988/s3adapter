...
todo = list(Path("/groups/virginiakm1988/LibriSpeech/train-clean-100").rglob("*")) 
print("test",todo)
#print(glob.glob("../../../../share/corpus_tar/LibriSpeech/*/",recursive=True))
exit()
##################
# BOOLEAB STRING #
##################
...
def generate_length(args, tr_set, audio_extension):
    ...
    for i, s in enumerate(tr_set):
        ...
        print('')
        #todo = list(Path("/groups/virginiakm1988/LibriSpeech/train-clean-100").rglob("*.flac")) 
        
        todo = list(Path(os.path.join(args.input_data, s)).rglob("*/*/*.flac")) # '*.flac'
        print(os.path.join(args.input_data, s))
