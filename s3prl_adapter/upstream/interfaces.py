...
class Featurizer(nn.Module):
    ...
    def _weighted_sum(self, feature):
        ...
        #print("stack_weight",stacked_feature.shape)
        # if "learned_weight" in sys.argv[-1]:
        #     adapter_bias = nn.Linear(stacked_feature.shape[1], 13)

        # print("stacked feature",stacked_feature.shape)
        norm_weights = F.softmax(self.weights, dim=-1)
        #print("norm_weights",norm_weights)
        weighted_feature = (norm_weights.unsqueeze(-1) * stacked_feature).sum(dim=0)
        # print("weighted feature",weighted_feature.shape)
        weighted_feature = weighted_feature.view(*origin_shape)
        # print("weighted feature",weighted_feature.shape)
        return weighted_feature#stacked_feature.sum(dim=0).view(*origin_shape)
    ...
    def forward(
        ...
        feature = self._select_feature(paired_features)
        #print("weighted feature suape", feature.shape)
        if isinstance(feature, (list, tuple)):
