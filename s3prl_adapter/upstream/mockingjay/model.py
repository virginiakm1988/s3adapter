...
class TransformerSelfAttention(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        if 'lora' in sys.argv[-1]:
            ...
            self.key = lora.Linear(config.hidden_size, self.all_head_size)
        else:
...
class TransformerOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        hidden_states = self.dense(hidden_states)
        houlsby_input = hidden_states
        hidden_states = self.dropout(hidden_states)

        if 'adapter' in sys.argv[1] and 'houlsby' not in sys.argv[-1] and 'lora' not in sys.argv[-1]:
                hidden_states = hidden_states + input_tensor +  self.adapter_vector  * self.adapter_alpha(adapter_input)
        elif 'houlsby' in sys.argv[-1]:
            hidden_states = hidden_states + input_tensor +  self.adapter(houlsby_input)
        else:
            hidden_states = hidden_states + input_tensor
